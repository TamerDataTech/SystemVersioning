...
var Modules = function () {
    ...
    var showModuleUpdate = function (moduleLog) {
        ...
        var options = {
            id: "set-module-log-modal",
            title: title,
            body: General.renderTemplate("#tmp-set-module-update", moduleLog),
            lock: true,
            buttons: [{
                style: "light-secondary",
                close: true,
            ...
            },
            {
                ...
                icon: "save",
                title: "Save",
                class: 'save-module-update',
                eventClass: 'save-module-update',
                onClick: function () {

                    var panel = $('#set-module-update-form');

                    if (General.validateForm(panel)) {
                        var modal = General.getFormModel(panel);

                        setModuleUpdate(modal, function () {
                            General.notifySuccess();
                            $('#set-module-log-modal').modal('hide');
                            Modules.refreshTable();
                        });
        ...
        var options = {
            ...
            },
            {
                ...
                onClick: function () {
                    ...
                    var logId = $('#module-version-update').val();

                    if (logId.isEmpty()) {
                        General.notifyFailure("Please choose a version and update to continue!");
                        return false;
                    }
    ...
    var setModuleUpdate = function (model, callback) {

        General.ajax({
            url: "/app/AddNewUpdate",
            data: model,
            panel: $('body'),
            success: function (result) {
    ...
    var prepareModulesEvents = function () {
        ...
        $('.new-update-module').off('click').on('click', function () {

            var id = $(this).attr("data-id");
            var version = $(this).attr("data-version");
            var update = $(this).attr("data-update");
            showModuleUpdate({ appModuleId: id, versionText: (parseInt(version)).padLeftZero(3), newUpdate: (parseInt(update) + 1).padLeftZero(3) });
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        "className": "text-center", "render": function (data, type, full, meta) {
                            return full.versionIndex.padLeftZero(3);
                        }
                    },
                    {
                        "name": "updateIndex",
                        "className": "text-center", "render": function (data, type, full, meta) {
                            return full.updateIndex.padLeftZero(3);
                        }
                    },
                    {
